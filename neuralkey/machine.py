...
from update_rules import LearningRule
import numpy as np
	...
	def update(self, tau2, update_rule='hebbian'):
		...
		if (tau1 == tau2):
			if update_rule == 'hebbian':
				self.rules.hebbian(W, X, sigma, tau1, tau2, l)
			elif update_rule == 'anti_hebbian':
				self.rules.anti_hebbian(W, X, sigma, tau1, tau2, l)
			elif update_rule == 'random_walk':
				self.rules.random_walk(W, X, sigma, tau1, tau2, l)
			else:
